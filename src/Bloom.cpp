...
//	bloomCam->setViewport(100, 100, 500, 500);
	...
	Camera* _noGlowCamera = new osg::Camera;		//the glow scene renders in our pre-render camera
	Camera* _glowCamera = new osg::Camera;
//	Camera* gc = _glowCamera;
	_glowCamera->setCullingMode(0);			//don't do any non-node-mask culling
	_noGlowCamera->setCullingMode(0);			//don't do any non-node-mask culling
...
//	gc = blendInputCam;
	_noGlowCamera->setClearMask(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);	//noglow draws first and clears everything
	_glowCamera->setClearMask(GL_COLOR_BUFFER_BIT);							//glow camera draws second, only clears color
	_noGlowCamera->setRenderOrder(osg::Camera::NESTED_RENDER, 4);
	_noGlowCamera->setClearColor(Vec4(1, 1, 0, 1));
	ShaderManager::applyShaderToNode("data/shaders/JustDepth", _noGlowCamera);		//use a shader to make everything the noglow camera draws black
	_glowCamera->setClearColor(Vec4(1, 1, 1, 1));
	_glowCamera->setRenderOrder(osg::Camera::NESTED_RENDER, 5);
	...
	_noGlowCamera->addChild(mScene);			//things that don't glow
	_noGlowCamera->setCullMask(1 << NON_GLOW_LAYER);
	_glowCamera->addChild(mScene);			//things in the scene do glow
	_glowCamera->setCullMask(1 << GLOW_LAYER | 1 << GLOW_ONLY_LAYER);
	...
	blendInputCam->addChild(_noGlowCamera);
	blendInputCam->addChild(_glowCamera);
	...
	glowBlendCam2->getOrCreateStateSet()->addUniform(mGlowAmount);
...
void BloomController::setGlowAmount(float f) {mGlowAmount->set(f);}
